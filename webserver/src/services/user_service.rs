...
use reqwest::{Client, blocking::Response, Error as ReqwestErr};
use serde_json::json;
use crate::models::email::{EmailRequest, Recipient, Sender};
...
pub async fn send_reset_email(email: &str, token: &str) -> Result<reqwest::Response, reqwest::Error> {
...
pub async fn send_verification_email(email: &str, code: &str) -> Result<(), reqwest::Error> {
    let smtp_token = std::env::var("SMTP_API_TOKEN").expect("SMTP_API_TOKEN must be set");
    let from_email = std::env::var("SMTP_FROM_EMAIL").expect("SMTP_FROM_EMAIL must be set");
    ...
    let body = serde_json::json!({
        ...
        "htmlContent": format!(
            "<p>Your verification code is: <strong>{}</strong>. It will expire in 5 minutes.</p>"
        , code)
    });
    ...
    client
        .post("https://api.brevo.com/v3/smtp/email")
        .header("api-key", smtp_token)
        .header("Content-Type", "application/json")
        .json(&body)
        .send()
        .await?; // ✅ 现在你可以 await，因为这是 async 请求
